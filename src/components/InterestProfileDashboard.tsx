    ...
    >
      ...
      <div className="flex items-center justify-between pb-3 border-b border-stone-200 dark:border-stone-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 id="profile-heading" className="text-sm font-bold text-stone-900 dark:text-stone-100">
        ...
        {topDomain && topDomain.score > 0 && (
          <div 
            className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-800 border border-indigo-200 dark:bg-indigo-950/50 dark:border-indigo-500/30 dark:text-indigo-300 text-xs"
            aria-label={`Leading technical domain: ${topDomain.label} at ${topDomain.score}%`}
          >
      ...
      <div 
        role="region"
        aria-label="Interest domain metrics"
        ...
        {profile.map((domain) => {
          ...
          return (
            <div 
              ...
              role="group"
              aria-label={`${domain.label}: ${domain.score}% weight${domain.growth > 0 ? `, +${domain.growth}% increase this interaction` : ''}`}
            >
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-2 font-mono text-[11px]">
                  {domain.growth > 0 && (
                    <span 
                      className="flex items-center gap-0.5 text-emerald-700 dark:text-emerald-400 font-semibold bg-emerald-50 dark:bg-emerald-950/60 px-1.5 py-0.2 rounded border border-emerald-200 dark:border-emerald-500/30"
                      aria-label={`Increased by ${domain.growth} percent`}
                    >
              ...
              {/* Progress bar */}
              <div 
                className="w-full h-2 rounded-full bg-stone-100 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 overflow-hidden relative"
                role="progressbar"
                aria-valuenow={domain.score}
                aria-valuemin={0}
                aria-valuemax={100}
                aria-label={`${domain.label} interest level`}
              >
                <div
      ...
      <div className="mt-1 pt-3 border-t border-stone-200 dark:border-stone-800/60 flex items-center justify-between text-[11px] text-stone-500 dark:text-stone-400">
        <span className="flex items-center gap-1">
          ...
          <span>Vectors update dynamically per interaction</span>
        </span>
