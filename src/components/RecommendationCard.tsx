...
import { Sparkles, ArrowRight, ShieldCheck, CheckCircle, Brain, Gauge, Tag, Compass, Layers, Code, Play } from 'lucide-react';
...
export const RecommendationCard: React.FC<RecommendationCardProps> = ({
  ...
  const recommendedReelData = reelsCatalog.find((r) => r.id === result.recommendedTechReel.id);
  ...
  return (
    <div className="bg-stone-900 border-2 border-indigo-500/30 rounded-2xl p-5 shadow-2xl relative overflow-hidden flex flex-col gap-5">
      {/* Subtle background glow */}
      <div className="absolute -right-24 -top-24 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -left-24 -bottom-24 w-64 h-64 bg-purple-500/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="bg-stone-950/70 border border-stone-800/80 rounded-xl p-3.5 flex flex-col gap-1.5">
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-semibold text-stone-400 uppercase tracking-wider flex items-center gap-1.5">
            ...
            AI Analytical Deduction (Beyond Surface Keywords)
          </span>
      ...
      <div className="bg-stone-950/90 border border-indigo-500/40 rounded-xl p-4 flex flex-col gap-3.5 shadow-inner">
        ...
        <div className="flex items-center justify-between text-xs text-stone-400 border-t border-stone-800/80 pt-2.5">
          <div className="flex items-center gap-2">
            ...
            {result.recommendedTechReel.duration && (
              <span className="font-mono text-stone-500">• {result.recommendedTechReel.duration}</span>
            )}
          </div>
          {result.recommendedTechReel.hashtags && result.recommendedTechReel.hashtags.length > 0 && (
            <div className="hidden sm:flex items-center gap-1.5 font-mono text-[11px] text-stone-500">
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            ...
            disabled={isLoading}
            className="flex-1 py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs flex items-center justify-center gap-2 shadow-md shadow-indigo-600/30 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="py-2.5 px-3 rounded-xl bg-stone-800 hover:bg-stone-700 text-stone-300 text-xs font-medium border border-stone-700 flex items-center gap-1.5 transition-colors cursor-pointer"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-stone-400">
            {result.reasoningSteps.map((step, idx) => (
              <div key={idx} className="flex items-start gap-1.5 p-1.5 rounded-lg bg-stone-900/50 border border-stone-800/50">
                <span className="w-4 h-4 rounded-full bg-indigo-500/20 text-indigo-400 font-mono text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                  {idx + 1}
                </span>
                <span className="leading-tight text-stone-300">{step}</span>
              </div>
            ))}
          </div>
        </div>
